...
const PixelBlast = ({
  ...
  useEffect(() => {
    ...
    if (mustReinit) {
      if (threeRef.current) {
        const old = threeRef.current;
        old.resizeObserver?.disconnect();
        cancelAnimationFrame(old.raf);
        old.quad?.geometry.dispose();
        old.material.dispose();
        old.composer?.dispose();
        old.renderer.dispose();
        old.renderer.forceContextLoss();
        if (old.renderer.domElement.parentElement === container) container.removeChild(old.renderer.domElement);
      ...
      threeRef.current = {
        ...
        touch,
        liquidEffect,
        pointerHandlers: { onPointerDown, onPointerMove },
      };
    } else {
    ...
    return () => {
      ...
      t.renderer.domElement.removeEventListener('pointerdown', t.pointerHandlers.onPointerDown);
      t.renderer.domElement.removeEventListener('pointermove', t.pointerHandlers.onPointerMove);
