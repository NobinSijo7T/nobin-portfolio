...
export default function SkillSet() {
    ...
    // GSAP Animations & Lenis Scroll-Driven Carousel Scroller
    useGSAP(() => {
        const boxes = gsap.utils.toArray(`.${styles.box}`);
        if (!boxes.length) return;

        const totalBoxes = boxes.length;
        const anglePerBox = 360 / totalBoxes;

        // Position boxes along the circular motion path
        const handleResize = () => {
            gsap.set(boxes, {
                motionPath: {
        ...
        // Helper: Find closest card to the collision / apex point and update active state
        const updateActiveCard = () => {
            if (!collisionDiv.current) return;
            const collisionRect = collisionDiv.current.getBoundingClientRect();
            ...
            boxes.forEach((box, index) => {
                ...
                const dist = Math.hypot(boxCenter.x - collisionCenter.x, boxCenter.y - collisionCenter.y);
                if (dist < minDistance) {
                    minDistance = dist;
                    closestIndex = index;
                }
        ...
        // ─── 1. Draggable vs Scroll State Management ────────────────────────
        const isDragging = { current: false };
        const dragOffsetRotation = { current: 0 };
        const scrollAngleSpan = 120; // Reduced from 360 to 120 for gentle, readable rotation on scroll

        let scrollTriggerInstance = null;

        // ─── 2. Draggable Integration (Snaps & Syncs with Scroll) ───────────
        setActiveIndex(0);
        const draggableInstance = Draggable.create(`.${styles.circularCarousel}`, {
            type: "rotation",
            inertia: true,
            snap: (endVal) => gsap.utils.snap(anglePerBox, endVal),
            onPress: () => {
            ...
            onThrowComplete: () => {
                ...
                // Compute offset so scroll-driven rotation continues seamlessly from where the user left it
                const currentRot = gsap.getProperty(`.${styles.circularCarousel}`, "rotation") || 0;
                const scrollProgress = scrollTriggerInstance ? scrollTriggerInstance.progress : 0;
                dragOffsetRotation.current = currentRot - (scrollProgress * scrollAngleSpan);
            },
        })[0];

        // ─── 3. Lenis / GSAP Scroll-Driven Carousel Scroller ─────────────────
        // Rotates the circular wheel gently as user scrolls through the section
        scrollTriggerInstance = ScrollTrigger.create({
            ...
            scrub: 1.5,
            onUpdate: (self) => {
                if (!isDragging.current && !draggableInstance?.isPressed && !draggableInstance?.isThrowing) {
                    const targetRot = (self.progress * scrollAngleSpan) + dragOffsetRotation.current;
        ...
        // ─── 4. Sphere & Atmosphere Parallax Scroller Effect ────────────────
        if (sphere.current) {
        ...
        // ─── 5. Blobs Parallax Drift (Reduced speed for subtle atmosphere) ──
        const blobEls = container.current?.querySelectorAll(`.${styles.blob}`);
        if (blobEls?.length) {
            blobEls.forEach((blob, i) => {
                gsap.to(blob, {
                    y: i % 2 === 0 ? -25 : -12, // Reduced travel distance for calmer motion
                    ease: "none",
                    scrollTrigger: {
                        trigger: container.current,
                        start: "top bottom",
                        end: "bottom top",
                        scrub: 3, // Slower, relaxed lag
                    }
                });
            });
        }

        // ─── 6. Title & Carousel Intro Reveal ───────────────────────────────
        const titleEl = container.current?.querySelector(`.${styles.grid} h1, .${styles.grid} h2, .${styles.grid} [class*="title"]`);
        if (titleEl) {
            gsap.fromTo(titleEl,
                { y: 40, autoAlpha: 0 },
                {
        ...
        const dragMeEl = container.current?.querySelector(`.${styles.dragMe}`);
        if (dragMeEl) {
            gsap.fromTo(dragMeEl,
                { autoAlpha: 0, x: 20 },
                {
                    autoAlpha: 1,
                    x: 0,
